...
def add_bit_1():
    ...
    unidad  = db((db.t_sustancias.id == sustancia)&(db.t_sustancias.f_estado == db.t_estado.id)).select(db.t_estado.ALL).first().f_estado
    if unidad == 'Sólido':
        unidad = 'g'
    else:
        unidad = 'cm3'
    ...
    if request.vars['t']=="Donación":
        ...
        #insert_bitacora_(_id,"Donación",0,cantidad)
        ...
        validar_bitacora_(unidad,request.now,sustancia,int(e_donante),cantidad,"Donación",descripcion)
